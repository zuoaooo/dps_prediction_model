...
print("\nPROPHET MODELS WITH PARAMETER TUNING")
...
for idx, (name, params) in enumerate(configs.items(), 1):
    ...
    try:
        ...
        models_predictions[f'Prophet_{name}'] = predictions
...
print("-" * 80)
print(f"{'Model':<35} {'MAE':<10} {'RMSE':<10} {'MAPE (%)':<12} {'Total Error':<12}")
print("-" * 80)
...
for model_name, predictions in models_predictions.items():
    ...
    print(f"{model_name:<35} {mae:>8.2f}   {rmse:>8.2f}   {mape:>10.2f}   {total_error:>10.2f}")
...
if best_model is not None:
    ...
    ax_best.set_title(f'Prophet: Best Model ({best_model}) Predictions vs Actual 2021',
                      fontsize=14, fontweight='bold')
    ax_best.grid(True, alpha=0.3)
    ax_best.legend(loc='best', fontsize=10)
    ax_best.tick_params(axis='x', rotation=45)
    plt.tight_layout()
    plt.savefig('prediction/prophet_best_model_vs_actual.png', dpi=300, bbox_inches='tight')
    plt.close(fig_best)
    print("\nBest model visualization saved to 'prediction/prophet_best_model_vs_actual.png'")
...
results_df.to_csv('prediction/prophet_prediction_results_2021.csv', index=False)
print("Detailed results saved to 'prediction/prophet_prediction_results_2021.csv'")
...
summary_df.to_csv('prediction/prophet_model_summary.csv', index=False)
print("Model summary (all models) saved to 'prediction/prophet_model_summary.csv'")
...
ax.set_title('Prophet Models: 2021 Predictions Comparison', fontsize=13, fontweight='bold')
ax.grid(True, alpha=0.3)
ax.legend(loc='best', fontsize=9)
ax.tick_params(axis='x', rotation=45)

plt.tight_layout()
plt.savefig('prediction/prophet_all_models_comparison.png', dpi=300, bbox_inches='tight')
print("Visualization saved to 'prediction/prophet_all_models_comparison.png'")
