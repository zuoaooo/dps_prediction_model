...
print("Testing different kernels and parameters to find optimal configuration\n")
print("This script compares various SVR configurations.")
...
# Parameter configurations for SVR
...
for idx, (name, params) in enumerate(configs.items(), 1):
    ...
    try:
        ...
        for month in range(1, 13):
            ...
            pred = svr_model.predict(last_row_scaled)[0]
...
if best_model is not None:
    ...
    ax_best.set_title(f'SVR: Best Model ({best_model}) Predictions vs Actual 2021',
                      fontsize=14, fontweight='bold')
    ax_best.grid(True, alpha=0.3)
    ax_best.legend(loc='best', fontsize=10)
    ax_best.tick_params(axis='x', rotation=45)
    plt.tight_layout()
    plt.savefig('prediction/svr_best_model_vs_actual.png', dpi=300, bbox_inches='tight')
    plt.close(fig_best)
    print("\nBest model visualization saved to 'prediction/svr_best_model_vs_actual.png'")
...
results_df.to_csv('prediction/svr_prediction_results_2021.csv', index=False)
print("Detailed results saved to 'prediction/svr_prediction_results_2021.csv'")
...
summary_df.to_csv('prediction/svr_model_summary.csv', index=False)
print("Model summary (all models) saved to 'prediction/svr_model_summary.csv'")
...
ax.set_title('SVR Models: 2021 Predictions Comparison', fontsize=13, fontweight='bold')
ax.grid(True, alpha=0.3)
ax.legend(loc='best', fontsize=9)
ax.tick_params(axis='x', rotation=45)

plt.tight_layout()
plt.savefig('prediction/svr_all_models_comparison.png', dpi=300, bbox_inches='tight')
print("Visualization saved to 'prediction/svr_all_models_comparison.png'")
